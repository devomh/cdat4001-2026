...
[Temporal Patterns: Trend, Seasonality & Noise](l17_concept_temporal_visualization.qmd).

[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/devomh/cdat4001-2026/blob/main/u05_visualization/l17_lab_san_juan_climate.ipynb)

> This page is the read-only view. To run the lab, open the notebook
> (`l17_lab_san_juan_climate.ipynb`) -- in Colab via the badge above, or
> locally. Every text output below comes from actually running the cells.

## Prerequisites & Setup

Run the setup cells first. One public-domain dataset, bundled with the
course and with a live download fallback:
...
- **Weather record:** NOAA NCEI Global Historical Climatology Network -
  ...
  web API with query parameters, exactly the L06 pattern.
...
```python
# Setup cell 2 of 2: imports + load the weather record (bundled, with a live fallback)
import os
...
> record. L10 filled gaps with a column's *median*, but for a time series
...
The climatology used the calendar as prior knowledge -- we *told* pandas to
group by month. But you can also *see* the cycle's length without assuming
it: overlay every year's twelve monthly values on one January-to-December
axis and watch the 65 curves pile onto the same hump.

```python
# Pivot to month-by-year, then draw one pale line per year + the climatology
pivot = temp_m.groupby([temp_m.index.year, temp_m.index.month]).mean().unstack(0)

fig, ax = plt.subplots(figsize=(8, 4))
for yr in pivot.columns:
    ax.plot(pivot.index, pivot[yr], color="steelblue", alpha=0.2, linewidth=0.8)
temp_clim.plot(ax=ax, color="crimson", linewidth=2.5, marker="o",
               label="climatology (the average cycle)")
ax.set_xlabel("month")
ax.set_ylabel("monthly mean temp (C)")
ax.set_title("Sixty-five years stacked: every year follows the same 12-month cycle")
ax.legend()
plt.tight_layout()
plt.show()
```

> **Read it:** 65 pale lines, one per year, and they trace the same arch --
> cool start, August peak, cool finish -- so the cycle's **period** is 12
> months, read straight off the picture without ever consulting the
> calendar. A repeating cycle like this is called **periodic**. Putting a
> single *number* on that self-similarity -- the series' correlation with a
> copy of itself shifted by 12 months, the **autocorrelation** -- needs the
> correlation coefficient, which arrives in **L19**; we will measure
> periodicity there. For now the overlay is proof enough.
...
The **z-score** -- L11's standardization -- re-expresses every value as "how
many standard deviations from this series' own mean", with L08's mean and
standard deviation as the ruler:
...
| See the period | overlay each year's cycle on one axis | The cycle repeats every 12 months (a number for it -- autocorrelation -- in L19) |
| Compare series | z-score; anomaly vs climatology | Normalization makes units comparable and de-seasonalizes |

Next lesson (L18): a linear-algebra primer -- the vectors and matrices the
rest of Unit V (correlation, PCA, clustering) is built on.

...
[Spatial Patterns: Principles, Coordinate Systems & Maps](l16_concept_spatial_visualization.qmd).

[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/devomh/cdat4001-2026/blob/main/u05_visualization/l16_lab_mapping_earthquakes.ipynb)

> This page is the read-only view. To run the lab, open the notebook
> (`l16_lab_mapping_earthquakes.ipynb`) -- in Colab via the badge above, or
> locally. Every text output below comes from actually running the cells.

## Prerequisites & Setup

Run the setup cells first. Two public-domain datasets, both bundled with the
course and both with live download fallbacks:
...
```python
# Setup cell 2 of 2: imports + load the two datasets (bundled, with live fallbacks)
import os
...
Color every municipio by a value -- one `column=` argument does it. We use a
**sequential** color ramp (`cmap="YlGnBu"`) because area is an ordered
magnitude, exactly as the concept note's color principle prescribes:
...
> color both encode magnitude (the double-encoding from the concept's first
> principle), so the handful of M5+ events jump out of the swarm of M3.5-4.5
> aftershocks.
...
back onto the boundary table -- an ordinary key-based merge does it (the
table-join concept from L05, realized in pandas as `merge`): match each
boundary row to its entry in
`counts` by municipio name (`left_on=` the name column, `right_index=` because
`counts` is indexed by name), keeping all 78 municipios (`how="left"`; the 72
with no match get NaN, hence `fillna(0)`).
...
sentence: what does the density map say that the count map cannot? (This is
the "avoid distortion" principle from the concept note, done by hand.)
...
and use a fact from this lab as evidence. (This is the concept's "choose the
chart to fit the question" principle as a judgment call.)
...
| Choropleth | `.plot(column=, legend=True)` | Sequential ramp for magnitude; choose the variable honestly |
...
Next lesson (L17): patterns in *time* -- trend, seasonality, and how to read
a variable that moves.
